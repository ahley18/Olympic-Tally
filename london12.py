...
#table
table = soup.find_all('table', class_='wikitable sortable sticky-header-multi plainrowheaders jquery-tablesorter')[0]
...
columns_to_keep = ['NOC', 'Gold', 'Silver', 'Bronze', 'Total']
...
df = df[['Rank', 'NOC', 'Gold', 'Silver', 'Bronze', 'Total']]

print(df)
df.to_csv(r'files\2012london.csv', index = False)
